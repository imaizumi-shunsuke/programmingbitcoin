...
class Tx:
    ...
    def parse(cls, s, testnet=False):
        ...
        version = little_endian_to_int(s.read(4)) # sはストリームなのでread()した分だけ進む
        
        num_inputs = read_varint(s) # ここではs.read(4)の後なので、ストリームの5バイト目からread_variant()している
        tx_ins = []
...
class TxIn:
    ...
    def parse(cls, s):
        ...
        script_sig = Script.parse(s) # これは6章で扱うのでここではそのまま使う。
